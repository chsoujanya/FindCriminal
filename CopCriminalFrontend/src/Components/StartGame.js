...
const StartGame = () => {
    ...
    const handleStartGame = async () => {
        try {
            const response = await axios.get('http://localhost:8080/game/start');
    ...
    const handleNextStep = async () => {
        if (step === 1 && selectedCities.length === 3) {
            ...
            try {
                const response = await axios.post('http://localhost:8080/game/check', { cops, criminalDetails });
                setCriminalCaught(true);
                setCaughtByCop(response.data.caughtByCop); // Update the cop who caught the criminal
                setCriminalCaughtCity(response.data.criminalCaughtCity); // Update the criminal's city
                setStep(3);
            } catch (error) {
    ...
    return (
        <div className="container-fluid" stylee = {{fontFamily: 'Times New Roman, Times,serif'}}>
    <div>
    ...
    <div style={{ textAlign: 'center' }}>
        ...
        {criminalCaught && caughtByCop && criminalCaughtCity ? (
            ...
            <p style={{ fontSize: '20px' }}>The criminal is not caught</p>
        )}
    </div>
)}
